...
async fn run_download_job(state: AppState, job_id: &str, urls: &str, output: Option<&str>, threads: u32) {
    ...
    let temp_file = temp_dir.join(format!("dpetoolbox_web_{}.txt", job_id));
    
    if let Err(e) = std::fs::write(&temp_file, urls) {
        state.update_job(job_id, |job| {
            job.status = JobStatus::Failed;
            job.message = format!("Failed to write temp file: {}", e);
        });
        return;
    }

    let result = crate::commands::download::run(
    ...
    state.update_job(job_id, |job| {
        match result {
            Ok(_) => {
                job.status = JobStatus::Completed;
                job.progress = 100;
                job.message = "Download completed successfully".to_string();
            }
            Err(e) => {
                job.status = JobStatus::Failed;
...
fn job_card_html(job: &super::state::Job) -> String {
    ...
    let output_html = if !job.output.is_empty() {
        let lines: String = job.output.iter()
            ...
            .collect();
        format!(r#"<div class="mt-2 bg-gray-900 text-green-400 p-2 rounded text-xs font-mono max-h-40 overflow-y-auto">{}</div>"#, lines)
    } else {
        String::new()
    };

    let stop_button = if job.job_type == "tcpping" && job.status == JobStatus::Running {
        format!(r#"<button hx-post="/api/tcpping/{}/stop" hx-swap="none" class="text-red-600 hover:text-red-800 text-sm">Stop</button>"#, job.id)
    } else {
        String::new()
    };

    format!(
        r#"<div class="bg-white rounded-lg shadow p-4 mb-3" id="job-{id}" 
             hx-get="/api/jobs/{id}" hx-trigger="every 1s" hx-swap="outerHTML">
...
<body class="bg-gray-100 min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-4xl">
        ...
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-cyan-600">DPE Toolbox</h1>
            <p class="text-gray-600">Network Analysis Toolbox</p>
        </div>

        <!-- Navigation Tabs -->
        <div class="bg-white rounded-lg shadow mb-6">
            <nav class="flex border-b">
                <button hx-get="/partials/download-form" hx-target="#form-container" 
                        class="px-4 py-3 text-sm font-medium text-gray-700 hover:text-cyan-600 hover:bg-gray-50 border-b-2 border-transparent hover:border-cyan-600">
                    Download
                </button>
                <button hx-get="/partials/merge-form" hx-target="#form-container"
                        class="px-4 py-3 text-sm font-medium text-gray-700 hover:text-cyan-600 hover:bg-gray-50 border-b-2 border-transparent hover:border-cyan-600">
                    Merge
                </button>
                <button hx-get="/partials/filter-form" hx-target="#form-container"
                        class="px-4 py-3 text-sm font-medium text-gray-700 hover:text-cyan-600 hover:bg-gray-50 border-b-2 border-transparent hover:border-cyan-600">
                    Filter
                </button>
                <button hx-get="/partials/convert-form" hx-target="#form-container"
                        class="px-4 py-3 text-sm font-medium text-gray-700 hover:text-cyan-600 hover:bg-gray-50 border-b-2 border-transparent hover:border-cyan-600">
                    Convert
                </button>
                <button hx-get="/partials/tcpping-form" hx-target="#form-container"
                        class="px-4 py-3 text-sm font-medium text-gray-700 hover:text-cyan-600 hover:bg-gray-50 border-b-2 border-transparent hover:border-cyan-600">
                    TCP Ping
                </button>
            </nav>
        ...
        <div class="bg-white rounded-lg shadow p-6">
            ...
            <div id="jobs-list" hx-get="/partials/jobs" hx-trigger="load, every 2s">
